...
// .wrangler/tmp/bundle-hit0P1/checked-fetch.js
...
var LibSQLPreparedQuery = class extends SQLitePreparedQuery {
  constructor(client, query, logger2, fields, tx, executeMethod, customResultMapper) {
...
function drizzle(client, config = {}) {
  ...
  const session2 = new LibSQLSession(client, dialect, schema2, { logger: logger2 }, void 0);
...
var dbMiddleware = createMiddleware(async (c2, next) => {
  c2.set(
    "db",
    getDB({
...
var luciaMiddleware = createMiddleware(async (c2, next) => {
...
// ../../shared/auth/rpc.ts
var authRoute = new Hono2().use("*", dbMiddleware).use("*", luciaMiddleware).use("*", sessionMiddleware).use(sessionMiddleware).use(luciaMiddleware).get("/login/github", async (c2) => {
...
var queueRoute = new Hono2().use(logger()).use("*", dbMiddleware).use("*", luciaMiddleware).use("*", sessionMiddleware).use("*", protectedMiddleware).use(dbMiddleware).use(luciaMiddleware).use(sessionMiddleware).use(protectedMiddleware).post("/join-queue", zValidator("json", zod3.queue.insert), async (c2) => {
  const db = c2.var.db;
  ...
  const playersInQueueWithSameGame = await db.query.queue.findMany({
    where: (queue2, { eq: eq2 }) => eq2(queue2.gameType, body.gameType)
  });
  if (playersInQueueWithSameGame.length === body.gameSize) {
    ...
    const subscriptions = await db.query.webpushSubscriptions.findMany({
      where: (sub, { inArray: inArray2 }) => inArray2(
        sub.userId,
        playersInQueueWithSameGame.map((p2) => p2.userId)
      )
    });
  ...
  async (c2) => {
    ...
    return c2.json({ status: "success" });
  }
);
var roomRoute = new Hono2().use("*", dbMiddleware).use("*", luciaMiddleware).use("*", sessionMiddleware).use("*", protectedMiddleware).use(dbMiddleware).use(luciaMiddleware).use(sessionMiddleware).use(protectedMiddleware).get(
  "/get-room-details",
  zValidator(
    "query",
    z.object({
...
// .wrangler/tmp/bundle-hit0P1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hit0P1/middleware-loader.entry.ts
